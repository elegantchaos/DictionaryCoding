...
fileprivate struct DictionaryCodingKeyedDecodingContainer<K : CodingKey> : KeyedDecodingContainerProtocol {
  ...
  internal func decodeInternal<T : Decodable>(_ type: T.Type, forKey key: Key, defaultValue : T) throws -> T {
    guard let entry = self.container[key.stringValue] else {
      if decoder.options.missingValueDecodingStrategy == .useDefault {
        return defaultValue
      }
    ...
    public func decode(_ type: Bool.Type, forKey key: Key) throws -> Bool {
      return try decodeInternal(type, forKey: key, defaultValue: false)
    }

    public func decode(_ type: Int.Type, forKey key: Key) throws -> Int {
      return try decodeInternal(type, forKey: key, defaultValue: 0)
    }
    ...
    public func decode(_ type: Float.Type, forKey key: Key) throws -> Float {
      return try decodeInternal(type, forKey: key, defaultValue: 0.0)
    }
    ...
    public func decode(_ type: String.Type, forKey key: Key) throws -> String {
      return try decodeInternal(type, forKey: key, defaultValue: "")
    }
